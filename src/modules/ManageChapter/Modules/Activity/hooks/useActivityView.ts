...
export const useActivityView = () => {
  ...
  }, [currentQuestion, selectedOptions, submittedAnswers, isActivityCompleted]);
  ...
  const onQuestionChange = useCallback(
    (index: number) => {
      // Allow navigation if:
      // 1. Activity is completed (view mode)
      // 2. All questions are submitted
      // 3. Navigating to already submitted question
      // 4. Activity is new/not started (no student_progress) - allow free navigation
      const hasStudentProgress = !!activityDetails?.data?.student_progress;
      if (
        isActivityCompleted ||
  ...
  const handleNextQuestionNavigation = useCallback(() => {
    if (isLastQuestion) {
      // Navigate back when on last question
      navigate(-1);
      return;
    }
  ...
  }, [activeQuestionIndex, isLastQuestion, totalQuestions, navigate]);
  ...
  const onOptionSelect = useCallback(
    (optionId: number) => {
      ...
      // Disable selection if activity is completed or question is submitted
      if (isActivityCompleted || submittedQuestions.has(activeQuestionIndex)) {
        return;
      }
    ...
    [currentQuestion, activeQuestionIndex, submittedQuestions, isActivityCompleted]
  );
  ...
  const onSubmitQuestion = useCallback(
    async (e?: React.MouseEvent) => {
      ...
      // Prevent submission if activity is already completed
  ...
  // Check if in view mode (all questions submitted or activity completed)
  // Don't enable view mode for new activities (no student_progress)
  // Also, don't enable view mode if there are unanswered questions (has_answer = false)
  const hasStudentProgress = !!activityDetails?.data?.student_progress;
  const hasUnansweredQuestions = questions?.some((q: any) => !q.has_answer) ?? false;
  const isViewMode =
    hasStudentProgress &&
    (isActivityCompleted || (areAllQuestionsSubmitted && !hasUnansweredQuestions));
