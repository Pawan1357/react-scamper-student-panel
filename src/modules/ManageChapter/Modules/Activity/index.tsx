...
import { DollarIcon, WalletIcon } from 'components/svg';
...
const ActivityViewPage = () => {
        ...
        return (
          <>
            ...
            <PairMatching
              ...
              userPairs={isViewMode ? pairMatchingSelections[pairQuestion.id] || [] : []}
              correctPairs={pairQuestion?.correct_pairs || []}
              isPastActivity={isPastActivity}
              hasUserAnswer={pairQuestion?.has_answer || false}
            />
            {!isSubmitted && !isActivityCompleted && (
              <SubmitButtonWrapper>
                <Button
  ...
  return (
    <>
      ...
      <HeaderToolbar
        title={TITLES.CLASSROOM.VIEW_ACTIVITY}
        backBtn
        backButtonHandler={goBackToLesson}
      />
      ...
      <ContentSection role="main">
        ...
        <QuestionSectionCard>
          {totalQuestions > 1 &&
            (stepper && stepper.length > 0 ? (
              <ProgressIndicator
                ...
                onClick={isViewMode ? navigateToQuestion : undefined}
              />
            ) : (
              <ProgressIndicator
                totalQuestions={totalQuestions}
          ...
          {isActivityCompleted && activityDetails?.data?.student_progress && (
            <ActivityScoreRow>
              {activityDetails?.data?.type !== 'drag_and_drop' && (
                <ActivityScorePill>
                  Points Earned:
              ...
              {activityDetails?.data?.type === 'drag_and_drop' &&
                currentQuestion?.fictitious_wallet_points &&
                currentQuestion?.fictitious_wallet_points === 'Yes' && (
                  <WalletBalanceContainer>
                    <WalletIcon />
                    <WalletBalanceLabel>Wallet Balance Left :</WalletBalanceLabel>
                    <WalletBalanceBadge>
                      {currentQuestion?.remaining_fictitious_wallet_balance ?? 0}
                      <DollarIcon />
                    </WalletBalanceBadge>
                  </WalletBalanceContainer>
                )}
        ...
        {modalData && !isActivityCompleted && (
          <AnswerResultModal
            ...
            walletInfo={modalData.walletInfo}
