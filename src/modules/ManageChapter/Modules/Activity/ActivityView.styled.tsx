...
export const DraggableItem = styled.div<{ $isDropped?: boolean; $isSubmitted?: boolean }>`
  ...
  max-width: 200px;
  min-height: 160px;
  background: ${(p) => (p.$isDropped ? '#fafafa' : '#f0f2f5')};
  border: ${(p) => (p.$isDropped ? '2px dashed #bfbfbf' : 'none')};
  border-radius: 18px;
  padding: 30px 12px 25px;
  text-align: center;
  box-shadow: ${(p) => (p.$isDropped ? 'none' : '0 4px 10px rgba(0, 0, 0, 0.08)')};
  position: relative;
  box-sizing: border-box;
  margin: 0 auto;
  opacity: ${(p) => (p.$isDropped ? 1 : 1)};
...
export const WalletBalanceBadge = styled.span`
  display: inline-flex;
  align-items: center;
  gap: 4px;
